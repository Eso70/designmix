...
export const ElegantProTemplate = memo(function ElegantProTemplate({
  ...
  const linksWithColors = useMemo(() => regularLinks.map((link) => ({ link, colors: getPlatformColors(link.platform, link.metadata?.custom_color as string | undefined) })), [regularLinks]);
